...
class Movie {
  ...
  static fromDoc(client, $) {
    ...
    $('div.pgs-search-wrap').each(function parseMovieBlock() {
      ...
      const movie = new self(client, { title, id, path, votes, rating });
  ...
  static fromAutocompleteResponse(client, response) {
    ...
    for (let i = 0; i < (response.data || []).length; i++) {
      ...
      const movie = new this(client, { title, id, path, votes, rating });
  ...
  static parseRating(ratingHtml) {
    let [, votes, rating] = ratingHtml.match(/:\s*(\d+)\s*"\s*>(\d+(?:\.\d+)?)</) || [];
